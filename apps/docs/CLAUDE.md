# CLAUDE.md - Protocol Launcher Docs

## Scope

- This file applies to `apps/docs`, the VitePress documentation app for Protocol Launcher.
- Keep the root `CLAUDE.md` in mind for workspace, package, and release rules; this file only adds docs-specific guidance.
...
- `packages/protocol-launcher` and `packages/shared` do not currently need their own nested `CLAUDE.md`; the root instructions already cover their build, test, export, and implementation rules.
- Add package-level Claude files later only if a package gains rules that differ materially from the root guidance.
