...
import { lingma } from '../src'

describe('lingma', () => {
  test('open should return a URL', async () => {
    const url = lingma.open()
  ...
  test('installMCP should return a URL with payload', async () => {
    ...
    expect(url).toBe(
      'lingma:mcp/install?%7B%22name%22%3A%22server-everything%22%2C%22type%22%3A%22stdio%22%2C%22command%22%3A%22npx%22%2C%22args%22%3A%5B%22-y%22%2C%22%40modelcontextprotocol%2Fserver-everything%22%5D%7D',
    )
  })
  ...
  test('installMCP should return a URL with payload and openInNewWindow', async () => {
    ...
    expect(url).toBe(
      'lingma:mcp/install?%7B%22name%22%3A%22server-everything%22%2C%22type%22%3A%22stdio%22%2C%22command%22%3A%22npx%22%2C%22args%22%3A%5B%22-y%22%2C%22%40modelcontextprotocol%2Fserver-everything%22%5D%7D&windowId=_blank',
    )
  })

  test('openFile should return a URL with path', async () => {
    const url = lingma.openFile({
      path: '/etc/hosts',
    })
    expect(url).toBe('lingma://file/etc/hosts')
  })
  ...
  test('openFile should return a URL with path, line, column, and openInNewWindow', async () => {
    ...
    expect(url).toBe('lingma://file/etc/hosts:1:2?windowId=_blank')
  })

  test('openFolder should return a URL with path and openInNewWindow', async () => {
    const url = lingma.openFolder({
      path: '/etc',
      openInNewWindow: true,
    })
    expect(url).toBe('lingma://file/etc?windowId=_blank')
  })
  ...
  test('openRemote should return a URL with type, host, and path', async () => {
    ...
    expect(url).toBe('lingma://vscode-remote/ssh-remote+root@172.18.105.209:22/code/my-project')
  })
  ...
  test('openRemote should return a URL with type, host, path and openInNewWindow', async () => {
    ...
    expect(url).toBe('lingma://vscode-remote/ssh-remote+root@172.18.105.209:22/code/my-project?windowId=_blank')
  })

  test('cloneProject should return a URL with repo', async () => {
    const url = lingma.cloneProject({
      repo: 'https://github.com/zhensherlock/protocol-launcher',
    })
    expect(url).toBe('lingma://vscode.git/clone?url=https%3A%2F%2Fgithub.com%2Fzhensherlock%2Fprotocol-launcher')
  })
})
