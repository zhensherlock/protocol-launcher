...
type OpenRemote = {
  /**
   * Remote type (Kiro Remote kind).
   *
   * Determines which Kiro Remote extension is used.
   *
   * - `ssh-remote`
 ...
 * })
 * // => 'kiro://vscode-remote/ssh-remote+root@172.18.105.209:22/code/my-project'
 */
export function openRemote(payload: OpenRemote) {
  const { type, host, path, openInNewWindow = false } = payload
  return `kiro://vscode-remote/${type}+${host}${path}${openInNewWindow ? '?windowId=_blank' : ''}`
}
