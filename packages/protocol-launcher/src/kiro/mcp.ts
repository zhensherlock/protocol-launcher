 ...
 * })
 * // => 'kiro://kiro.mcp/add?name=server-everything&config=%7B...%7D&windowId=_blank'
 */
export function installMCP(payload: MCPServer) {
  const { openInNewWindow = false, ...config } = payload
  const { name, ...mcpConfig } = config
  const encodedPayload = encodeURIComponent(JSON.stringify(mcpConfig))
  return `kiro://kiro.mcp/add?name=${name}&config=${encodedPayload}${openInNewWindow ? '&windowId=_blank' : ''}`
}
