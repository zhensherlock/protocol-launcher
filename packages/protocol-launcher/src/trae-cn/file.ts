...
/**
 * Open file in Trae China.
 *
 * @param payload Open file definition.
 * @returns Trae open file URL.
 * @example
 * openFile({
 *   path: '/etc/hosts',
 *   line: 10,
 *   column: 5,
 * })
 * // => 'trae-cn://file/etc/hosts:10:5'
 */
export function openFile(payload: OpenFile) {
  const { path, line, column, openInNewWindow = false } = payload
  return `trae-cn://file${path}${isUndefined(line) ? '' : `:${line}`}${isUndefined(column) ? '' : `:${column}`}${openInNewWindow ? '?windowId=_blank' : ''}`
}
