...
type OpenRemote = {
  /**
   * Remote type (Trae China Remote kind).
   *
   * Determines which Trae China Remote extension is used.
   *
   * - `ssh-remote`
...
/**
 * Open remote in Trae China.
 *
 * @param payload Open remote definition.
 * @returns Trae open remote URL.
 * @example
 * openRemote({
 *   type: 'ssh-remote',
 ...
 * })
 * // => 'trae-cn://vscode-remote/ssh-remote+root@172.18.105.209:22/code/my-project'
 */
export function openRemote(payload: OpenRemote) {
  const { type, host, path, openInNewWindow = false } = payload
  return `trae-cn://vscode-remote/${type}+${host}${path}${openInNewWindow ? '?windowId=_blank' : ''}`
}
