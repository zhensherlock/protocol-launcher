...
type OpenRemote = {
  /**
   * Remote type (CodeBuddy Remote kind).
   *
   * Determines which CodeBuddy Remote extension is used.
   *
   * - `ssh-remote`
...
/**
 * Open remote in CodeBuddy China.
 *
 * @param payload Open remote definition.
 * @returns CodeBuddy China open remote URL.
 * @example
 * openRemote({
 *   type: 'ssh-remote',
 ...
 * })
 * // => 'codebuddycn://vscode-remote/ssh-remote+root@172.18.105.209:22/code/my-project'
 */
export function openRemote(payload: OpenRemote) {
  const { type, host, path, openInNewWindow = false } = payload
  return `codebuddycn://vscode-remote/${type}+${host}${path}${openInNewWindow ? '?windowId=_blank' : ''}`
}
