...
type OpenRemote = {
  /**
   * Remote type (VS Code Insiders Remote kind).
   *
   * Determines which VS Code Insiders Remote extension is used.
   *
   * - `ssh-remote`
...
/**
 * Open remote in VS Code Insiders.
 *
 ...
 * })
 * // => 'vscode-insiders://vscode-remote/ssh-remote+root@172.18.105.209:22/code/my-project'
 */
export function openRemote(payload: OpenRemote) {
  const { type, host, path, openInNewWindow = false } = payload
  return `vscode-insiders://vscode-remote/${type}+${host}${path}${openInNewWindow ? '?windowId=_blank' : ''}`
}
