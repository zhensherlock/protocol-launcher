...
type OpenRemote = {
  /**
   * Remote type (VSCodium Remote kind).
   *
   * Determines which VSCodium Remote extension is used.
   *
   * - `ssh-remote`
...
/**
 * Open remote in VSCodium.
 *
 * @param payload Open remote definition.
 * @returns VSCodium open remote URL.
 * @example
 * openRemote({
 *   type: 'ssh-remote',
 ...
 * })
 * // => 'vscodium://vscode-remote/ssh-remote+root@172.18.105.209:22/code/my-project'
 */
export function openRemote(payload: OpenRemote) {
  const { type, host, path, openInNewWindow = false } = payload
  return `vscodium://vscode-remote/${type}+${host}${path}${openInNewWindow ? '?windowId=_blank' : ''}`
}
