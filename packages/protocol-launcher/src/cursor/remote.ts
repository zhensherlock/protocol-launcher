...
type OpenRemote = {
  /**
   * Remote type (Cursor Remote kind).
   *
   * Determines which Cursor Remote extension is used.
   *
   * - `ssh-remote`
...
/**
 * Open remote in Cursor
 *
 * @param payload Open remote definition.
 * @returns Cursor Open Remote URL.
 * @example
 * openRemote({
 *   type: 'ssh-remote',
 ...
 * })
 * // => 'cursor://vscode-remote/ssh-remote+root@172.18.105.209:22/code/my-project'
 */
export function openRemote(payload: OpenRemote) {
  const { type, host, path, openInNewWindow = false } = payload
  return `cursor://vscode-remote/${type}+${host}${path}${openInNewWindow ? '?windowId=_blank' : ''}`
}
