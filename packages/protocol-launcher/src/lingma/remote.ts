...
type OpenRemote = {
  /**
   * Remote type (Lingma Remote kind).
   *
   * Determines which Lingma Remote extension is used.
   *
   * - `ssh-remote`
...
/**
 * Open remote in Lingma
 *
 * @param payload Open remote definition.
 * @returns Lingma open remote URL.
 * @example
 * openRemote({
 *   type: 'ssh-remote',
 ...
 * })
 * // => 'lingma://vscode-remote/ssh-remote+root@172.18.105.209:22/code/my-project'
 */
export function openRemote(payload: OpenRemote) {
  const { type, host, path, openInNewWindow = false } = payload
  return `lingma://vscode-remote/${type}+${host}${path}${openInNewWindow ? '?windowId=_blank' : ''}`
}
