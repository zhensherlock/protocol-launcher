...
type OpenRemote = {
  /**
   * Remote type (Qoder Remote kind).
   *
   * Determines which Qoder Remote extension is used.
   *
   * - `ssh-remote`
...
/**
 * Open remote in Qoder.
 *
 * @param payload Open remote definition.
 * @returns Qoder open remote URL.
 * @example
 * openRemote({
 *   type: 'ssh-remote',
 ...
 * })
 * // => 'qoder://vscode-remote/ssh-remote+root@172.18.105.209:22/code/my-project'
 */
export function openRemote(payload: OpenRemote) {
  const { type, host, path, openInNewWindow = false } = payload
  return `qoder://vscode-remote/${type}+${host}${path}${openInNewWindow ? '?windowId=_blank' : ''}`
}
