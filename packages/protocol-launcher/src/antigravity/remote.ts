...
type OpenRemote = {
  /**
   * Remote type (Antigravity Remote kind).
   *
   * Determines which Antigravity Remote extension is used.
   *
   * - `ssh-remote`
...
/**
 * Open remote in Antigravity.
 *
 * @param payload Open remote definition.
 * @returns Antigravity open remote URL.
 * @example
 * openRemote({
 *   type: 'ssh-remote',
 ...
 * })
 * // => 'antigravity://vscode-remote/ssh-remote+root@172.18.105.209:22/code/my-project'
 */
export function openRemote(payload: OpenRemote) {
  const { type, host, path, openInNewWindow = false } = payload
  return `antigravity://vscode-remote/${type}+${host}${path}${openInNewWindow ? '?windowId=_blank' : ''}`
}
