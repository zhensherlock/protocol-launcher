...
type OpenRemote = {
  /**
   * Remote type (PearAI Remote kind).
   *
   * Determines which PearAI Remote extension is used.
   *
   * - `ssh-remote`
...
/**
 * Open remote in PearAI
 *
 * @param payload Open remote definition.
 * @returns PearAI open remote URL.
 * @example
 * openRemote({
 *   type: 'ssh-remote',
 ...
 * })
 * // => 'pearai://vscode-remote/ssh-remote+root@172.18.105.209:22/code/my-project'
 */
export function openRemote(payload: OpenRemote) {
  const { type, host, path, openInNewWindow = false } = payload
  return `pearai://vscode-remote/${type}+${host}${path}${openInNewWindow ? '?windowId=_blank' : ''}`
}
